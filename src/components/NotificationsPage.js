...
/**
 * NotificationsPage — Shared full notifications page for ALL roles.
 * Used by /account/notifications (customer), and vendor/retailer dashboards
 * so every user type sees the same polished notification experience.
 * Reads/writes the real /api/notifications backend.
 */
export default function NotificationsPage({ role = 'customer', title = 'Notifications' }) {
  ...
  const notifIcon = (type) => {
    const icons = {
      ...
      order_new: '🛒', dispute_update: '⚖️', payout: '💸',
    };
    return icons[type] || '🔔';
  };
  ...
  return (
    <DashboardLayout role={role}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-ob-navy">{title}</h1>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {[
          ...
          { value: 'kyc_update', label: 'Verification' },
