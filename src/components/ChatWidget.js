...
export default function ChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    const userMsg = { id: Date.now().toString(), role: 'user', content: input.trim() || (attachedImage ? 'Please look at this screenshot' : ''), image: attachedImage || null };
  ...
  return (
    <>
      {!open && (
        <button onClick={() => setOpen(true)} className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-ob-purple hover:bg-ob-purple-dark text-white rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center" title="Chat with OjaBridge AI">
      ...
      {open && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[380px] h-[calc(100vh-4rem)] sm:h-[560px] max-h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="bg-ob-navy px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between flex-shrink-0">
            ...
            <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-white transition-colors p-1"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-ob-purple text-white rounded-br-md' : 'bg-white text-gray-700 border border-gray-100 shadow-sm rounded-bl-md'}`}>
                  {msg.image && <div className="mb-2"><img src={msg.image} alt="Attached" className="rounded-lg max-h-36 sm:max-h-40 w-auto object-cover" /></div>}
                  {msg.role === 'assistant' ? <div className="space-y-0.5">{renderMessage(msg.content)}</div> : <p className="whitespace-pre-wrap">{msg.content}</p>}
                </div>
              </div>
            ))}
            {loading && (
          ...
          {/* Emoji Picker — Grid that actually works */}
          {showEmoji && (
            <div ref={emojiRef} className="border-t border-gray-100 bg-white max-h-48 overflow-y-auto">
              <EmojiGrid onSelect={handleEmojiSelect} />
            </div>
          )}

          {/* Input */}
          <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-t border-gray-100 bg-white flex-shrink-0">
            <div className="flex items-end gap-1.5 sm:gap-2">
              <button onClick={() => setShowEmoji(!showEmoji)}
                className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-lg transition-colors flex-shrink-0 ${showEmoji ? 'bg-ob-purple/10 text-ob-purple' : 'text-gray-400 hover:text-ob-purple hover:bg-gray-100'}`} title="Emoji">
                <span className="text-lg sm:text-xl">😊</span>
              </button>
              <button onClick={() => fileInputRef.current?.click()}
                className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-ob-purple hover:bg-gray-100 transition-colors flex-shrink-0" title="Attach image">
              ...
              <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
                ...
                className="flex-1 resize-none border border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-sm focus:border-ob-purple outline-none max-h-20"
            ...
            <p className="text-[9px] sm:text-[10px] text-gray-300 mt-1.5 sm:mt-2 text-center">OjaBridge AI • awoyoemmanuel12@gmail.com</p>
          </div>
        </div>
      )}
    </>
