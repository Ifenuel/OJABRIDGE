    ...
    <DashboardLayout role="admin" requiredPermission="users">
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-bold text-ob-navy">All Users ({filteredUsers.length})</h3>
          <ExportButton
            dateRangeOptions={dateRangeOptions}
            onExport={({ format, dateRange }) => {
              const exportRows = dateRange ? filterByDateRange(filteredUsers, dateRange.start, dateRange.end, 'created_at') : filteredUsers;
              exportData({
                format,
                title: 'Users Report',
                filename: 'ojabridge_users',
                dateRange,
                summary: [
