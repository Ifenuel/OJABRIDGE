  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (dispRes.status === 'fulfilled' && dispRes.value.success) setDisputes(dispRes.value.disputes || []);
    ...
    try {
      ...
      if (data.success) {
        setDisputes(prev => prev.map(d =>
          d.id === disputeId ? { ...d, messages: [...(d.messages || []), { sender: 'retailer', message: newMessage, created_at: new Date().toISOString() }] } : d
        ));
        setNewMessage('');
      }
    ...
    <DashboardLayout role="retailer">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Report and track issues with your orders</p>
        </div>
          ...
          <form onSubmit={handleCreateDispute} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <select required value={createForm.order_id}
              ...
              <div>
                ...
                <select required value={createForm.reason}
            ...
            <div>
              ...
              <textarea required rows={4} value={createForm.description}
      ...
      {loadingData ? (
        ...
        <div className="bg-white p-12 rounded-2xl border border-gray-100 text-center">
          ...
          <Link href="/retailer-dashboard/orders" className="text-ob-purple text-sm font-medium hover:underline">View your orders →</Link>
        </div>
      ) : (
              ...
              {dispute.messages && dispute.messages.length > 0 && (
                <div className="border-t border-gray-100 pt-4 mt-4 space-y-3">
                  ...
                  {dispute.messages.map((msg, idx) => (
                    <div key={idx} className={`p-3 rounded-lg ${msg.sender === 'retailer' ? 'bg-ob-purple/5 ml-8' : 'bg-gray-50 mr-8'}`}>
                      <p className="text-xs font-medium text-gray-500 mb-1">
                        {msg.sender === 'retailer' ? 'You' : msg.sender === 'admin' ? 'OjaBridge Support' : 'Vendor'}
