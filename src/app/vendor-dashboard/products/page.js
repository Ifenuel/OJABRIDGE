...
export default function VendorProductsPage() {
  ...
  const loadProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/products?vendor=${user?.id || ''}&limit=50`);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setForm({ name: '', description: '', shortDescription: '', price: '', compareAtPrice: '', category: '', stock: '', sku: '', weight: '', tags: '' });
        loadProducts();
      } else {
  ...
  return (
    <DashboardLayout role="vendor">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        ...
        <button onClick={() => setShowAddForm(!showAddForm)} className="btn-primary text-sm px-5 py-2.5 inline-flex items-center gap-2">
              ...
              {loading ? (
                ...
                filteredProducts.map((product) => (
                  <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">{product.vendor_profiles?.store_name || 'Your Store'}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusBadge(product.status)}`}>
